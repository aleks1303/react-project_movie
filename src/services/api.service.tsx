...
const movieService = {
  getAllMovie: async (): Promise<IMovies[]> => {
    const {
      data: { results },
    } = await axiosInstance.get(`/discover/movie&api_key=${apiKey}`);
    return results;
  },
  ...
  getAllGenres: async (): Promise<IGenres[]> => {
    const {
      data: { genres },
    } = await axiosInstance.get(`/genre/movie/list?api_key=${apiKey}`);
    return genres;
  },
  getMoviesByGenre: async (genreId: number): Promise<IMovies[]> => {
    const {
      ...
      `/discover/movie?with_genres=${genreId}&api_key=${apiKey}`,
    );
    return results;
  },
  searchMovieByQuery: async (
    query: string,
    page: string,
  ): Promise<IMovies[]> => {
    const {
      ...
      `/search/movie?query=${query}&page=${page}&api_key=${apiKey}`,
    );
    return results;
  },
};
