  ...
  return (
    <div className={"p-3 bg-gray-100 flex gap-6"}>
      ...
      <div>
        ...
        <div className={"mt-2 flex gap-2 flex-wrap"}>
          {movie.genres.map((g) => (
            <span
              key={g.id}
              className={
                "text-sm bg-gray-600 text-white px-2 py-1 rounded-full"
              }
            >
              {g.name}
            </span>
        ...
        <p className={"mt-4 bg-gray-300"}>
          Country:{" "}
          {movie.production_countries.map((country, index) => (
            <CountryProductComponent key={index} country={country} />
          ))}
        </p>
        <p className={"mt-4"}>
          Product:{" "}
        ...
        <p className={"mt-4 bg-gray-300"}>
          ...
          {movie.spoken_languages.map((language, index) => (
            <LanguageComponent key={index} language={language} />
          ))}
        </p>
